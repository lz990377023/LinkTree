...
- (BOOL)lt_isMobileNumberClassificationVerify {
    ...
    if (([self lt_isValidateByRegex:CM])
        || ([self lt_isValidateByRegex:CU])
        || ([self lt_isValidateByRegex:CT])
        || ([self lt_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
- (BOOL)lt_isMobileNumberVerify {
    ...
    BOOL ret1 = [self lt_isValidateByRegex:mobileRegex];
    return ret1;
}
...
- (BOOL)lt_isEmailAddressVerify {
    ...
    return [self lt_isValidateByRegex:emailRegex];
}

//身份证号
- (BOOL)lt_isIdentityCardVerify {
    NSString *regex2 = @"^(\\d{14}|\\d{17})(\\d|[xX])$";
    return [self lt_isValidateByRegex:regex2];
}
...
- (BOOL)lt_isCarNumberVerify {
    ...
    return [self lt_isValidateByRegex:carRegex];
}

- (BOOL)lt_isMacAddressVerify {
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self lt_isValidateByRegex:macAddRegex];
}

- (BOOL)lt_isValidUrlVerify {
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self lt_isValidateByRegex:regex];
}

- (BOOL)lt_isValidChineseVerify {
    NSString *chineseRegex = @"^[\u4e00-\u9fa5]+$";
    return [self lt_isValidateByRegex:chineseRegex];
}

- (BOOL)lt_isValidPostalcodeVerify {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self lt_isValidateByRegex:postalRegex];
}

- (BOOL)lt_isValidTaxNoVerify {
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self lt_isValidateByRegex:taxNoRegex];
}
    ...
    switch (length) {
        ...
        case 18:
            ...
            if(numberofMatch >0) {
                ...
                NSString *test = [number substringWithRange:NSMakeRange(17,1)];
...
@implementation NSString (LTBase64)

+ (NSString *)lt_stringWithBase64EncodedString:(NSString *)string {
    NSData *data = [NSData lt_dataWithBase64EncodedString:string];
    if (data)
    {
...
- (NSString *)lt_base64EncodedStringWithWrapWidth:(NSUInteger)wrapWidth {
    ...
    return [data lt_base64EncodedStringWithWrapWidth:wrapWidth];
}

- (NSString *)lt_base64EncodedString {
    NSData *data = [self dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    return [data lt_base64EncodedString];
}

- (NSString *)lt_base64DecodedString {
    return [NSString lt_stringWithBase64EncodedString:self];
}

- (NSData *)lt_base64DecodedData {
    return [NSData lt_dataWithBase64EncodedString:self];
}
